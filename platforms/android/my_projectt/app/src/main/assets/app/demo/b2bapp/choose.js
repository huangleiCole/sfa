...
__vue_options__.__file = "E:\\Users\\Cole\\weex_project\\my_projectt\\src\\demo\\b2bapp\\choose.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-01824810"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
exports.default = {
  ...
  created: function created() {
    var that = this;
    var dataparam = [{ 'name': "method", 'value': "b2bapp.sales.choose_member" }, { 'name': "app_key", 'value': "b2bapp" }, { 'name': "format", 'value': "json" }, { 'name': "datetime", 'value': _http2.default.formatTime(new Date()) }, { 'name': "v", 'value': '2.0' }, { 'name': "sales_staff_id", 'value': pref.get('user').sales_staff_id }];
    _http2.default.post(dataparam, function (res) {
      var reslist = res.data;
      var tranList = JSON.parse("[]");
      var vList = JSON.parse("[]");
      for (var index in reslist) {
        if (reslist[index].weekwork == "本周已完成") {
          vList[vList.length] = reslist[index];
          reslist[index].daywork = "已处理";
        }
      }

      console.log(reslist);
      for (var index in reslist) {
        if (reslist[index].daywork == "今日已完成") {
          tranList[tranList.length] = reslist[index];
        }
      }

      for (var index in reslist) {
        if (reslist[index].daywork == "今日未完成") {
          tranList[tranList.length] = reslist[index];
        }
      }

      that.list = tranList;
      that.visitedlist = vList;
  ...
  methods: {
    ...
    jump: function jump(sales_staff_id, member_id, member_name, area, addr, event) {
      var nav = weex.requireModule('navigator');
      var param = {};
      param.sales_staff_id = sales_staff_id;
    ...
    jump_b: function jump_b(sales_staff_id, member_id, member_name, area, addr, event) {
      var nav = weex.requireModule('navigator');
      var param = {};
      param.sales_staff_id = sales_staff_id;
      param.member_id = member_id;
      ...
      param.area = area;
      param.addr = addr;
      nav.pushParam('./executed.js', param);
    }
  }
};
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    staticClass: ["img3"],
    attrs: {
      "src": "http://47.98.186.145/api/web/static/img/map.png"
    }
  })]), _vm._l((_vm.list), function(item, index, key) {
    return _c('div', [_c('div', {
      staticClass: ["gird_row"],
      on: {
        "click": function($event) {
          _vm.jump(item.sales_staff_id, item.member_id, item.member_name, item.area, item.addr, $event)
        }
      }
    }, [_c('div', [_c('text', {
      staticClass: ["text2"]
    }, [_vm._v(_vm._s(item.member_name))]), _c('text', {
      staticClass: ["text5"]
    }, [_vm._v(_vm._s(item.area))]), _c('text', {
      staticClass: ["text5"]
    }, [_vm._v(_vm._s(item.addr))]), _c('text', {
      staticClass: ["text6"]
    }, [_vm._v("完成度： " + _vm._s(item.visited_count) + " / " + _vm._s(item.visit_count))])]), (item.weekwork == '本周已完成') ? _c('div', {
      staticClass: ["item_right"]
    }, [_c('div', [_c('image', {
      staticClass: ["img2"],
      attrs: {
        "src": "http://47.98.186.145/api/web/static/img/tick.png"
      }
    })])]) : (item.daywork == '今日已完成') ? _c('div', {
      staticClass: ["item_right"]
    }, [_c('div', {
      staticClass: ["item_right"]
    }, [_c('image', {
    ...
    staticClass: ["gird_column"]
  }, _vm._l((_vm.visitedlist), function(item, index, key) {
    return _c('div', [_c('div', {
      staticClass: ["gird_row"],
      on: {
        "click": function($event) {
          _vm.jump_b(item.sales_staff_id, item.member_id, item.member_name, item.area, item.addr, $event)
        }
      }
    }, [_c('div', [_c('text', {
      staticClass: ["text2"]
